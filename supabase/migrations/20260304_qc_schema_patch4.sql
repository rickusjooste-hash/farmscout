-- Patch 4: add farm_id to orchards in get_qc_reference_data
-- Needed so the QC runner app can set the correct farm_id on a bag
-- session when GPS fails and the orchard is selected manually from
-- a list that spans multiple farms.
