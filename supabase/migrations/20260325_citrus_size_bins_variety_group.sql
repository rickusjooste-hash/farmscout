-- Add optional variety_group to size_bins for dual-system commodities (e.g., citrus: mandarin vs lemon)
-- Bins with variety_group = NULL match any orchard of that commodity (backward compatible).
-- Bins with a specific value only show for orchards with matching variety_group.
ALTER TABLE size_bins ADD COLUMN IF NOT EXISTS variety_group text;

-- Update get_qc_reference_data to include variety_group in orchards output.
-- (size_bins uses json_agg(sb ...) which auto-includes new columns)
