-- Per-orchard severity for a specific pest with optional week parameters
-- Backwards compatible: NULL params default to current ISO week
-- Run in Supabase SQL Editor

CREATE OR REPLACE FUNCTION get_tree_orchard_severity(
  p_farm_ids   uuid[],
  p_pest_id    uuid,
...
LANGUAGE sql SECURITY DEFINER AS $$
  ...
  -- Per-orchard raw aggregates for the given pest within the date range
  tw_orchard AS (
    SELECT
      os.id                                                    AS orchard_id,
    ...
    WHERE obs.pest_id = p_pest_id
      AND s.inspected_at >= COALESCE(p_week_start, date_trunc('week', now()))::timestamptz
      AND s.inspected_at <  COALESCE(p_week_end,   date_trunc('week', now()) + interval '7 days')::timestamptz
