-- ============================================================
-- Performance rewrite: avoid scanning 2.3M qc_fruit rows on
-- wide date ranges. Pre-aggregate fruit counts per session in
-- a materialized CTE, then join only the aggregates.
-- Also adds p_commodity_id to KPIs, bag list, picker breakdown.
-- Run in Supabase SQL Editor.
-- ============================================================

-- ── 1. get_qc_dashboard_kpis ──────────────────────────────────────────────

DROP FUNCTION IF EXISTS get_qc_dashboard_kpis(uuid[], timestamptz, timestamptz, uuid);
...
RETURNS json LANGUAGE sql SECURITY DEFINER AS $$
  ...
  -- Pre-aggregate fruit per session (avoids scanning all 2M+ fruit rows)
...
DROP FUNCTION IF EXISTS get_qc_bag_list(uuid[], timestamptz, timestamptz, uuid);
...
LANGUAGE sql SECURITY DEFINER AS $$
  -- First get the 200 most recent sessions (cheap — only bag_sessions table)
  ...
  -- Only aggregate fruit/issues for these 200 sessions
  fruit_agg AS (
...
DROP FUNCTION IF EXISTS get_qc_picker_issue_breakdown(uuid[], timestamptz, timestamptz, uuid);
