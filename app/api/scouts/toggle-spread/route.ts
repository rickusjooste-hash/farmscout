...
export async function POST(req: NextRequest) {
  ...
  const { scoutId, enforceGpsSpread, gpsSpreadPin } = body
  if (!scoutId) {
  ...
  if (gpsSpreadPin !== undefined && gpsSpreadPin !== null && gpsSpreadPin !== '') {
    if (!/^\d{4}$/.test(gpsSpreadPin)) {
  ...
  const { error } = await svc
    .from('scouts')
    .update({
      enforce_gps_spread: !!enforceGpsSpread,
      gps_spread_pin: enforceGpsSpread ? (gpsSpreadPin || null) : null,
    })
    .eq('id', scoutId)
