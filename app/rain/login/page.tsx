...
export default function RainLogin() {
  ...
  async function handleLogin() {
    ...
    try {
      ...
      // 2. Look up profile + org + farm access
      const headers = {
      ...
      localStorage.setItem('rainapp_access_token', accessToken)
      localStorage.setItem('rainapp_refresh_token', authData.refresh_token)
      localStorage.setItem('rainapp_user_id', userId)
      localStorage.setItem('rainapp_user_name', profiles[0].full_name)
      localStorage.setItem('rainapp_farm_id', farmId)
      localStorage.setItem('rainapp_farm_ids', JSON.stringify(farmAccess.map((f: any) => f.farm_id)))
      localStorage.setItem('rainapp_org_id', orgId)

      // 4. Pull reference data into IndexedDB
      const { pullRainReferenceData, pullRecentReadings } = await import('@/lib/rain-sync')
      await pullRainReferenceData(accessToken)
      await pullRecentReadings(accessToken)

      // 5. Navigate to RainApp home
  ...
  return (
    <div className="flex flex-col justify-center min-h-dvh w-full bg-[#eae6df] px-6">
      <div className="text-center mb-12 max-w-[480px] w-full self-center">
        ...
        <div className="text-4xl font-extrabold tracking-wide text-[#2176d9]">allFarm Rain</div>
        <div className="text-sm text-[#8a95a0] mt-1 tracking-widest uppercase">Rainfall Capture</div>
      </div>
      ...
      <div className="text-center text-xs text-[#c0bab4] mt-8">allFarm Rain v1.0</div>
    </div>
  )
}
