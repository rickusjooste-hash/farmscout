...
export default function PackshedWeighLogin() {
  ...
  async function handleLogin() {
    ...
    try {
      ...
      // 3. localStorage
      localStorage.setItem('packshedweigh_access_token', accessToken)
      localStorage.setItem('packshedweigh_refresh_token', authData.refresh_token)
      localStorage.setItem('packshedweigh_user_id', userId)
      localStorage.setItem('packshedweigh_user_name', profiles[0].full_name)
      localStorage.setItem('packshedweigh_farm_id', farmId)
      localStorage.setItem('packshedweigh_farm_ids', JSON.stringify(farmAccess.map((f: any) => f.farm_id)))
      localStorage.setItem('packshedweigh_org_id', orgId)

      // 4. Pull reference data
      const { pullWeighReferenceData, pullTodayWeighRecords } = await import('@/lib/packshed-weigh-sync')
      await pullWeighReferenceData(accessToken)
      await pullTodayWeighRecords(accessToken)

      // 5. Navigate
      window.location.href = '/packshed/weigh'
    } catch (err: any) {
  ...
  return (
    <div className="flex flex-col justify-center min-h-dvh w-full bg-[#eae6df] px-6">
      <div className="text-center mb-12 max-w-[480px] w-full self-center">
        ...
        <div className="text-4xl font-extrabold tracking-wide text-[#2176d9]">allFarm Weigh</div>
        <div className="text-sm text-[#8a95a0] mt-1 tracking-widest uppercase">Bin Weighing</div>
      </div>
      ...
      <div className="text-center text-xs text-[#c0bab4] mt-8">allFarm Weigh v1.0</div>
    </div>
  )
}
