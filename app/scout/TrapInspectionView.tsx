    ...
    <div style={styles.app}>
      ...
      <div style={styles.screen}>
        <div style={styles.pestImageContainer}>
          {trap.pest?.image_url ? (
            <img src={trap.pest.image_url} alt={trap.pest.name} style={styles.pestImage} />
          ) : (
        ...
        <div style={styles.section}>
          ...
          <div style={styles.countInput}>
            <button style={styles.countBtn} onClick={() => setCount(c => Math.max(0, c - 1))}>−</button>
        ...
        <div style={styles.section}>
          ...
          <div style={styles.rebaitRow}>
            <button
              style={{ ...styles.rebaitBtn, background: !rebaited ? '#3a1a1a' : 'transparent', borderColor: !rebaited ? '#e05c4b' : '#3a4228', color: !rebaited ? '#e05c4b' : '#7a8a5a' }}
              onClick={() => setRebaited(false)}
            >No</button>
            <button
              style={{ ...styles.rebaitBtn, background: rebaited ? '#1a3a1a' : 'transparent', borderColor: rebaited ? '#6abf4b' : '#3a4228', color: rebaited ? '#6abf4b' : '#7a8a5a' }}
        ...
        <div style={styles.gpsStatus}>
          <span style={{ ...styles.gpsDot, background: gpsLocation ? '#6abf4b' : '#7a8a5a' }} />
          {gpsLocation
...
const styles: Record<string, React.CSSProperties> = {
  app: { display: 'flex', flexDirection: 'column', height: '100dvh', width: '100%', maxWidth: 600, margin: '0 auto', background: '#1a1f0e', color: '#e8e8d8', fontFamily: 'system-ui, sans-serif', overflow: 'hidden' },
  header: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px', background: '#222918', borderBottom: '1px solid #3a4228', flexShrink: 0 },
  headerTitle: { fontSize: 18, fontWeight: 700 },
  backBtn: { background: 'none', border: 'none', color: '#f0a500', fontSize: 22, cursor: 'pointer', padding: 0 },
  progressText: { fontSize: 13, color: '#7a8a5a', fontVariantNumeric: 'tabular-nums' },
  progressBar: { height: 4, background: '#3a4228', flexShrink: 0 },
  ...
  pestImageContainer: { display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '24px 16px 16px', borderBottom: '1px solid #3a4228' },
  pestImage: { width: 160, height: 160, objectFit: 'contain', borderRadius: 8, background: '#222918', padding: 8 },
  pestImagePlaceholder: { width: 160, height: 160, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 60, background: '#222918', borderRadius: 8 },
  pestName: { marginTop: 10, fontSize: 16, fontWeight: 600, color: '#e8e8d8' },
  trapInfo: { padding: '12px 16px', borderBottom: '1px solid #3a4228' },
  infoRow: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingBottom: 8 },
  infoLabel: { fontSize: 12, color: '#7a8a5a', textTransform: 'uppercase', letterSpacing: '0.08em' },
  infoValue: { fontSize: 14, fontWeight: 500, color: '#e8e8d8', textAlign: 'right', maxWidth: '65%' },
  rebaitWarning: { margin: '12px 16px', padding: '12px 16px', background: '#2a1f00', border: '1px solid #f0a500', borderRadius: 6, color: '#f0a500', fontSize: 14, fontWeight: 600 },
  section: { padding: '16px 16px 0' },
  sectionLabel: { fontSize: 11, fontWeight: 600, letterSpacing: '0.12em', textTransform: 'uppercase', color: '#7a8a5a', marginBottom: 10 },
  countInput: { display: 'flex', alignItems: 'center', border: '1px solid #3a4228', borderRadius: 6, overflow: 'hidden' },
  countBtn: { width: 64, height: 64, background: '#222918', border: 'none', color: '#f0a500', fontSize: 28, fontWeight: 600, cursor: 'pointer', flexShrink: 0 },
  countValue: { flex: 1, textAlign: 'center', fontSize: 36, fontWeight: 700, color: '#e8e8d8', background: '#1f2514', height: 64, display: 'flex', alignItems: 'center', justifyContent: 'center' },
  rebaitRow: { display: 'flex', gap: 12 },
  rebaitBtn: { flex: 1, padding: '14px', borderRadius: 6, border: '1px solid', fontSize: 16, fontWeight: 700, cursor: 'pointer', letterSpacing: '0.05em', transition: 'all 0.15s' },
  gpsStatus: { display: 'flex', alignItems: 'center', gap: 6, padding: '16px', fontSize: 11, color: '#7a8a5a' },
  gpsDot: { width: 7, height: 7, borderRadius: '50%', flexShrink: 0 },
  bottomBar: { display: 'flex', gap: 12, padding: 16, background: '#222918', borderTop: '1px solid #3a4228', flexShrink: 0 },
  cancelBtn: { flex: 1, padding: '14px', background: 'transparent', border: '1px solid #3a4228', borderRadius: 6, color: '#7a8a5a', fontSize: 16, fontWeight: 600, cursor: 'pointer' },
  saveBtn: { flex: 2, padding: '14px', background: '#f0a500', border: 'none', borderRadius: 6, color: '#000', fontSize: 16, fontWeight: 700, cursor: 'pointer' },
  secondaryBtn: { padding: '12px 24px', background: 'transparent', border: '1px solid #3a4228', borderRadius: 6, color: '#e8e8d8', fontSize: 15, fontWeight: 600, cursor: 'pointer' },
}
