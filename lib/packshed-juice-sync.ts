...
export async function pullJuiceReferenceData(
  ...
  try {
    ...
    const [packhouseRes, orchardRes, defectRes] = await Promise.all([
      ...
      fetch(
        `${SUPABASE_REST}/commodity_pests?category=eq.qc_issue&is_active=eq.true&select=pest_id,display_name,display_name_af,display_order,commodity_id&order=display_order`,
        { headers }
      ),
    ])
